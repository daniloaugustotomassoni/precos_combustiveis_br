...
class PCBRGasolina(scrapy.Spider):
    name = 'pcbrg'
    ...
    def parse(self, response: Response, **kwargs: Any) -> Any:
        for i in range(len(self.start_urls)):
            if(self.start_urls[i] == response.url):
                uf = self.UFS[i]
                preco_medio_final = response.xpath('//*[@id="telafinal-precofinal"]/text()').get()
                distribuicao_revenda = response.xpath('//*[@id="quadrobrancofinal"]/div[2]/div[1]/div/p[3]/span/text()').get()
                custo_etanol_anidro = response.xpath('//*[@id="quadrobrancofinal"]/div[2]/div[2]/div/p[3]/span/text()').get()
                ...
                yield {
                        ...
                        'porcentagens':{
                            ...
                            'custo_etanol_anidro':self.float_parse(custo_etanol_anidro),
